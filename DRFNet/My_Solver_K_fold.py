...
class Solver:
    def __init__(self, device, batch_size, input_size, dropout_rate, learning_rate, num_class, D, out_channels,
                 eeg_groups, sa_groups, initial_weight_decay=2e-4):
        self.device = device
...
# new metrics
    ...
    def compute_model_costs(self, input_size, batch_size=8):  # 增大batch_size
        ...
        self.flops, self.params = profile(self.myModel, inputs=(input_tensor, dummy_label), verbose=False)
    ...
    def train_epoch(self):
        ...
        for batch_idx, (data_src, label_src) in enumerate(self.train_loader):
            ...
            # data_src = data_src.permute(0, 3, 1, 2)
            self.opt.zero_grad()
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for batch_idx, (data, label) in enumerate(self.val_loader):
                ...
                globalf = self.myModel.global_enc(rele)
                logit = self.myModel.classifier(globalf)
    ...
    def test(self):
        ...
        return acc, f1, precision, recall, cm, precision_weighted, recall_weighted, f1_weighted, kappa, mcc# ,dbi, ivr, cosine_sim
